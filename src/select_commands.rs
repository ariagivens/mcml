...
pub fn select_commands(program: assign_homes::Program) -> Result<Vec<Function>> {
    ...
    for (i, t) in program.tests.iter().enumerate() {
        ...
        functions.push(Function { namespace: "mctest".to_owned(), name: format!("test{i}"), content });
...
fn select_commands_stmt(stmt: Statement, test_name: &str) -> String {
    match stmt {
        Statement::Assert { atom: Atom::LitBool(b) } => format!(
            "tellraw @s \"{} - {}\"\n",
            if b { "ok" } else { "not ok" },
            escape(test_name)
        ),
        Statement::Assert { atom: Atom::Location(Location::Stack { offset }) } => {
            let ok = format!("execute if score {offset} stack matches 1.. run tellraw @s \"ok - {test_name}\"");
            let not_ok = format!("execute unless score {offset} stack matches 1.. run tellraw @s \"not ok - {test_name}\"");
            format!("{ok}\n{not_ok}\n")
        }
        Statement::Assert { atom: Atom::LitInt(i) } => panic!("Type error! Expected <bool> saw <int>"),
        Statement::AssertEq { left: Atom::LitBool(left), right: Atom::LitBool(right) } =>
            format!(
                "tellraw @s \"{} - {}\"\n",
                if left == right { "ok" } else { "not ok" },
                escape(test_name)
            ),
        Statement::AssertEq { left: Atom::LitInt(left), right: Atom::LitInt(right) } =>
            format!(
                "tellraw @s \"{} - {}\"\n",
                if left == right { "ok" } else { "not ok" },
                escape(test_name)
            ),
        Statement::AssertEq { left: Atom::Location(Location::Stack { offset }), right: Atom::LitInt(i) } => {
                let ok = format!("execute if score {offset} stack matches {i} run tellraw @s \"ok - {test_name}\"");
                let not_ok = format!("execute unless score {offset} stack matches {i} run tellraw @s \"not ok - {test_name}\"");
                format!("{ok}\n{not_ok}\n")
            }
        Statement::AssertEq { left: Atom::LitInt(i), right: Atom::Location(Location::Stack { offset }) } => {
            let ok = format!("execute if score {offset} stack matches {i} run tellraw @s \"ok - {test_name}\"");
            let not_ok = format!("execute unless score {offset} stack matches {i} run tellraw @s \"not ok - {test_name}\"");
            format!("{ok}\n{not_ok}\n")
        }
        Statement::AssertEq { left: Atom::Location(Location::Stack { offset: left_offset }), right: Atom::Location(Location::Stack { offset: right_offset }) } => {
        ...
        Statement::Assign { location: Location::Stack { offset }, expr } => {
...
fn scratch_atom(atom: Atom) -> String {
    match atom {
        Atom::Location(Location::Stack { offset }) => format!("scoreboard players operation scratch stack = {} stack\n", offset),
        Atom::LitInt(i) => format!("scoreboard players set scratch stack {i}\n"),
        Atom::LitBool(b) => format!("scoreboard players set scratch stack {}\n", if b { 1 } else { 0 }),
    }
}
